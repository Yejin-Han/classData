...
function Write(){
	...
	const writeComplete = async(e) => {
		e.preventDefault();
		try{
			await axios.post("http://localhost:8080/board", list);
			navigate("/");
		} catch(err){
			console.log(err);
		}
		...
		<div>
			<Form>
				<Form.Group className="mb-3" controlId="title" name="title">
					...
					<Form.Control type="text" name="title" placeholder="제목을 입력하세요." onChange={handleChange} /> {/* onChange={(e)=>{setList(list.title=e.target.value)}} */}
				</Form.Group>
